...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:8080/api/admin/profile?email=${email}`)
        ...
        <div className="chat-popup">
          ...
          <div className="chat-users">
            ...
            {organizers.map(org => (
              <div
                key={org.id}
                className={`chat-user ${selectedUser === org.id ? "active" : ""}`}
                onClick={() => {
                  const id = org.id;
                  setSelectedUser(id);
                  loadMessages(id);
          ...
          <div className="chat-body">
            ...
            {!selectedUser ? (
              <p style={{ fontSize: "12px" }}>Select an organizer to view messages</p>
            ) : (
              ...
              messages
                ...
                .map((msg) => (
                  <div
                    key={msg.id}
                    className={msg.senderId && Number(msg.senderId) === senderId
                      ? "my-message"
                      : "other-message"}
                  >
                    {msg.message}
