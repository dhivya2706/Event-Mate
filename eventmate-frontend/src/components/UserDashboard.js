...
const isUpcomingEvent = (event) => {
  ...
  return eventDate >= today; // only today + future
};
...
function UserDashboard() {
  ...
  useEffect(() => {
    ...
    const iv = setInterval(() => setCurrentIndex(p => p === upcomingEvents.length - 1 ? 0 : p + 1), 3500);
  ...
  const goToBooking = (event) => {
    ...
    // Case 1: Already full event (from dashboard)
    if (event.eventName) {
    ...
    // Case 2: ChatBot event → find matching full event
    const matchedEvent = events.find(
      e => e.eventName === event.name
    );

    if (matchedEvent) {
      setSelectedEventData(matchedEvent);
      setActivePage("bookings");
    } else {
 ...
 const submitFeedback = async () => {
  ...
  const eventId =
    selectedFeedbackEvent.event?.id || selectedFeedbackEvent.eventId;
  ...
  try {
    ...
    // ✅ REMOVE EVENT FROM UI (THIS IS THE KEY LINE)
    setFeedbackEvents(prev =>
      prev.filter(ev => ev.id !== selectedFeedbackEvent.id)
    );
  ...
  const displayedEvents = selectedDate
    ? events.filter(e => e.eventDate === selectedDate && isUpcomingEvent(e))
      ...
      <div className="app-shell">
              ...
              <div className="content-card">
                ...
                {upcomingEvents.length > 0 && currentEvent && (
                  <div className="slider-container">
                    ...
                    <img
                      src={`http://localhost:8080/uploads/${currentEvent.imageName}`}
                    ...
                    <div className="slider-overlay">
                      ...
                      <p>VIP: {currentEvent.vipSeats} | ₹{currentEvent.vipPrice}</p>
                      <p>Premium: {currentEvent.premiumSeats} | ₹{currentEvent.premiumPrice}</p>
                      <p>Regular: {currentEvent.regularSeats} | ₹{currentEvent.regularPrice}</p>
                ...
                <div className="events-grid">
                  {events.filter(isUpcomingEvent).slice(0, 6).map(ev => <EventCard key={ev.id} event={ev} onClick={setDetailEvent} />)}
                </div>
              </div>
            )}
                  ...
                  : (
                    <div className="fb-grid">
                      {feedbackEvents.map(ev => {
                        const imageName = ev.event?.imageName || ev.imageName;
                        ...
                        return (
                          <div key={ev.id} className="fb-card">
                            ...
                            <div className="fb-img-strip">
                              <img
                                src={imageName ? `http://localhost:8080/uploads/${imageName}` : "/fallback.jpg"}
                                alt="event"
                                className="fb-img"
                                onError={(e) => {
                            ...
                            <div className="fb-card-body">
                              <h3 className="fb-event-name">
                                {ev.event?.eventName || ev.eventName || "Event Name"}
                              </h3>
                              ...
                              <button
                                className="fb-review-btn"
                                onClick={() => setSelectedFeedbackEvent(ev)}
                              >
                                Write a Review
                              </button>
                            </div>
        ...
        {showChatBot && (
          <ChatBot
            ...
            events={events.filter(isUpcomingEvent)}
          />
        )}
      </div>
    </div>
